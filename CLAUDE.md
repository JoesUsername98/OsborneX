...
Pure-Python (stdlib only, no pybind11/native extension) package `osbornex`, standalone `pyproject.toml` under `Python/` — not wired into the root CMake build. Reimplements the wire protocol (`Net/inc/Net/wire/*.hpp`) byte-for-byte in `struct` (little-endian, no htons-style conversion, matching the C++ side's x86-64-only assumption) and orchestrates the existing `ServerMain`/`BotMain` executables as subprocesses rather than reimplementing the engine. `osbornex.Server.start()`/`Bot.start()` discover built binaries under `build/*/{Server,Bot}/**/`; `stop()` writes a newline to the subprocess's stdin (both mains already read a line from stdin to exit), falling back to `terminate()`/`kill()`. Order ids use the same source-namespacing scheme as `Bot::RandomStrategy` (top 16 bits `source`, low 48 bits a local counter) — bots and the Python client must be given distinct explicit `source` values or their order ids can collide. `Strategy` mirrors `Bot::RandomStrategy`'s callback pattern (`on_top_of_book`/`on_trade`, `buy`/`sell`/`cancel`). Order scope is add + cancel only (no modify). Dependency/environment management is via [`uv`](https://docs.astral.sh/uv/) (`uv sync` installs the project plus the `dev` dependency group -- currently `pytest` and `trademetrics` -- into `Python/.venv`, tracked by `Python/uv.lock`); build backend is `uv_build` rather than `setuptools`, specifically because it doesn't leave an `egg-info/` scratch directory in the source tree the way `setuptools` does. Tests: `Python/tests/`, run via `uv run pytest` from `Python/`; the end-to-end test skips cleanly with a message if no CMake preset has been built yet. `Strategy.buy`/`sell` track their own returned order ids so `Strategy.my_fill(trade)` can tell whether a `TradeExecution` was one of this strategy's own orders getting filled (vs. two other participants trading) -- returns an `OwnFill(side, price, quantity)` or `None`.

`Python/strategies/` holds your own working scripts (not shipped/canonical); `Python/examples/` holds the library's own unmodified reference sample. Both are driver scripts that compose the `osbornex` library for one scenario -- that composition logic doesn't belong inside the library itself.

### TradeMetrics (`TradeMetrics/`)

Standalone Python package `trademetrics`, its own `pyproject.toml`/`uv.lock`/`.venv` at the repo root, sibling to `Python/` -- **zero dependency on `osbornex` or anything trading-system-specific**, by design (per explicit request: PnL/drawdown/performance metrics belong in a generic module, not one coupled to this specific exchange). The only input type is `Fill` (timestamp, symbol, side, price, quantity) -- feed it fills from anywhere. `PositionTracker` does average-cost realized-PnL accounting per symbol (opening/adding to a position books no PnL, only reducing/closing/reversing one does); `build_equity_curve` replays fills into a cumulative-realized-PnL curve (optionally marking open positions to market via a caller-supplied price function); `drawdown`/`returns`/`trades` compute max drawdown, drawdown duration, Sharpe/Sortino/volatility, and win-rate/profit-factor/average-win/average-loss from that curve; `summarize()`/`format_report()` bundle all of it into one `PerformanceReport`. `Python/pyproject.toml` depends on it as a local editable path (`[tool.uv.sources] trademetrics = { path = "../TradeMetrics" }`) purely as a dev convenience for strategy scripts -- the integration point (converting a `TradeExecution`/`OwnFill` into a `trademetrics.Fill`) lives in `Python/strategies/my_strategy.py`, not in either library. Tests: `TradeMetrics/tests/`, run via `uv run pytest` from `TradeMetrics/`.
